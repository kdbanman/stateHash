 ...
 * first stage.
 *
 * @param {Object} error the error (or null) generated at the insertion stage
 ...
 */
var handleInsert = function (error, results, client, time_writing, fn) {
    if (error) {
    ...
    getClient(function(error, client) {
        if (error) fn(error);
            ...
            client.query(preparedInsert, function(error, results) {
                handleInsert(error, results, client, time_writing, fn);
            });
        }
    });
};
...
var perHashCountText = "SELECT count(*) " +
 ...
 */
storage.discrepancyCountTotal(algorithm, fn)
{
    fn(null, 0);
}
...
var perSeedCountText = "SELECT seed, count($1) " +
                       "FROM reports " +
                       "GROUP BY seed " +
                       "HAVING count($1) > 1;";
 ...
 */
storage.discrepancyCountPerSeed(algorithm, fn)
{
    fn(null, {});
}
 ...
 * @param {Function} fn callback, passed error/null and map containing
 *                      size, hash_algorithm, user_agent, object
 */
storage.discrepancyCount(algorithm, fn)
{
    fn(null, {});
}
