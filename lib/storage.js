...
var perHashCountText =
    "SELECT count(*) as total " +
    "FROM (SELECT seed, count(DISTINCT {{HASH}}) as discrepancies " +
          "FROM reports " +
          "GROUP BY seed " +
          "HAVING count(DISTINCT {{HASH}}) > 1) hash_discrepancies;";
...
{
    // prevent sql injection by throwing on whitespace or semicolon
    if (algorithm.search(/[\s;]/) !== -1)
        fn(new Error('bad algorithm requested!'));

    getClient(function(err, client) {
        if (err) fn(err);
        else {
            // construct column name and query
            var col = 'hash_' + algorithm;
            var queryText = perHashCountText.replace(/{{HASH}}/g, col);
...
var perSeedCountText =
    "SELECT seed, count(DISTINCT {{HASH}}) as discrepancy_count " +
    "FROM reports " +
    "GROUP BY seed " +
...
storage.discrepancyCountPerSeed = function (algorithm, fn)
{
    ...
    getClient(function(err, client) {
        if (err) fn(err);
            ...
            var col = 'hash_' + algorithm;
            var queryText = perSeedCountText.replace(/{{HASH}}/g, col);
...
var seedReportsName = "seed {{HASH}} hashcodes";
var seedReportsText = "SELECT size, {{HASH}}, user_agent, object " +
                      "FROM reports " +
                      "WHERE seed = $1 " +
                      "ORDER BY {{HASH}};";
...
storage.seedReports = function (algorithm, seed, fn)
{
    ...
    getClient(function(err, client) {
        if (err) fn(err);
            ...
            // construct column name
            var col = 'hash_' + algorithm;
            var queryName = seedReportsName.replace(/{{HASH}}/g, col);
            var queryText = seedReportsText.replace(/{{HASH}}/g, col);
