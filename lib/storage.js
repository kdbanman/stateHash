...
var perHashCountText = "SELECT count(*) as total " +
                       "FROM (SELECT seed, count($1) as discrepancies " +
                             "FROM reports " +
                             "GROUP BY seed " +
                             "HAVING discrepancies > 1) hash_discrepancies;";
    ...
    getClient(function(err, client) {
        if (err) fn(err);
            ...
            var preparedQuery = {name: perHashCountName,
                                 ...
                                 values: [col]};
            console.log("Performing prepared query:");
            console.log(JSON.stringify(preparedQuery));
